...
- `src/core/` — pure, dependency-free logic: data types, bit fields, field-shape
  ...
  array `view` window), `FieldColors` (raw-view highlight ranges),
  `FormatOrder` (canonical top-level key order), `FormatConstants` (named
  `constants` values, e.g. for an array's `countField`), the parser,
