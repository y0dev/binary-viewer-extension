...
| `npm run package` | Production bundle + `vsce package` → `binary-structure-inspector-<version>.vsix` |
...
| `node scripts/gen-screenshots.js` | Re-render `docs/images/*.png` from the `docs/mockups/*.html` mockups via headless Chrome/Edge (set `CHROME_PATH` if not auto-found) |
...
3. `vsce publish` (requires the `devdoesit` Marketplace publisher).
